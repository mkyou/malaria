...
formulas = list(
  ...
  # formula4 drops rhum since it was not significant in formula2/4 fits
  Y ~ f(mes, model = 'rw2', constr = T, cyclic = T) +
    f(ano, model = 'rw1', constr = T) +
    f(idArea, model = 'bym2', graph = micro_path) +
    f(idMes, model = 'rw1') +
...
for (i in seq_along(formulas)) {
  message('Fitting bell falciparum model ', i)
  fit = inla(
    formula = formulas[[i]], family = 'bell', data = micro_f,
  ...
  save_model_selection_row(
    family = 'bell', species = 'falciparum', model_id = i, fit = fit
  )
  rm(fit)
  gc()
}
