...
df_falciparum_pre = df_falciparum_pre |>
  mutate(new_poi_preds = ifelse(
    ano == 2018 & poisson_preds < 1,
    poisson_preds*fit$coefficients[1],

    ifelse(
      ano == 2018 & poisson_preds >= 1 & poisson_preds < 4,
      poisson_preds*fit2$coefficients[1],

      ifelse(
        ano == 2018 & poisson_preds >= 4,
...
ggsave('results/legacy/erros_falciparum_2018.png')
...
ggsave('results/legacy/erros_falciparum_2018_corrigido.png')
...
test_errors_falciparum |> 
  write_csv('results/legacy/test_metrics_microrregion_falciparum.csv')

df_falciparum_pre |> write_csv('results/legacy/preds_microrregion_falciparum_df.csv')
